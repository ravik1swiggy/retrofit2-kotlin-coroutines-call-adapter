...
sealed class Error(val tid: String? = null, val sid: String? = null) {
	class BlackZoneError(val title: String?, val message: String?, tid: String?, sid: String?) :
		Error(tid, sid)

	class DescriptiveError(val code: Int, val message: String, tid: String?, sid: String?) :
		Error(tid, sid)
	...
	class UnhandledExceptionError(val error: Throwable? = null, val message: String? = null) :
		Error()
